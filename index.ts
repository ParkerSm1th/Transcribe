...
const addTranslatedTextToVideo = async (
  ...
  const videoText = translatedParts
    .map((part, index) => {
      ...
      return `drawtext=font='Sans':box=1:boxcolor=black@0.5:fontsize=38:alpha:0.5:fontcolor=white:x=(w-text_w)/2:y=h-th-140:text='${escapedText}':enable='between(t,${startTime},${endTime})'`;
    })
    .join(",");
